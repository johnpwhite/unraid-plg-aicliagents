...
if [ -f "$CONFIG_FILE" ]; then
    ...
    if [ -d "$NEW_PERSIST" ]; then
        status "Pre-populating RAM Home for $USER_NAME from persistence (Forced Upgrade Sync)..."
        mkdir -p "$RAM_HOME"
        # D-52/D-54: Performance: Exclude heavy, non-essential caches/logs and use --size-only.
        EXCLUDES="--exclude='.npm' --exclude='.bun' --exclude='.cache' --exclude='node_modules' --exclude='*.log' --exclude='log/'"
        if rsync -avcL --delete --size-only $EXCLUDES "$NEW_PERSIST/" "$RAM_HOME/"; then
