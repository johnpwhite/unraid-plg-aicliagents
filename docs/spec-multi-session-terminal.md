...
**Project:** AICliAgents CLI Unraid Plugin  
**Version:** 1.0 (March 2026)  
**Status:** Implemented (v2026.03.02.06)

## 1. Overview
This feature provides a robust, multi-tabbed terminal interface for the AICliAgents CLI AI agent within the Unraid 7.2+ WebGUI. It enables persistent, resumable AI chat sessions mapped to specific filesystem paths, surviving both tab navigation and server reboots.
...
- **Process Isolation:** Each tab is assigned a unique alphanumeric ID (e.g., `sXXXXX`). This ID is used to create:
    - A dedicated Unix Socket: `/var/run/aicliterm-$id.sock`
    - A dedicated Tmux Session: `aicli-agent-$id`
    - A dedicated PID file: `/var/run/unraid-aicliagents-$id.pid`
- **Persistence Policy:** 
    - **Ephemeral State:** `/var/run` tracks active processes but is wiped on reboot.
    - **Persistent Data:** The plugin's `home_path` (on the Unraid flash drive) stores the AICliAgents CLI's `.gemini` folder, containing `projects.json` and `tmp/` (logs/history).
- **Session "Reality" Tracking:** The backend tracks the active `chatSessionId` for each running `ttyd` instance in `/var/run/unraid-aicliagents-$id.chatid`.
...
**Solution:**
1.  **Upward Path Traversal:** The backend matches filesystem paths to AICli projects by traversing upward (e.g., `/mnt/user/python/app` -> `/mnt/user/python`). It validates that a project folder actually exists in `tmp/` before claiming a match.
2.  **Short-ID Extraction:** Extracted the 8-character resumable prefix from the full UUID found in `logs.json` to satisfy the `aicli --resume` command requirements.
3.  **Live Status Polling:** The UI polls `/plugins/unraid-aicliagents/AICliAjax.php?action=get_session_status` every 4 seconds. This returns the "Live Truth" from the `logs.json` file on disk. If the user switches sessions inside the terminal, the UI updates automatically.
...
- **Tab Metadata Overlay:** Hovering over a tab shows the full path and the active AICli `chatSessionId`.
